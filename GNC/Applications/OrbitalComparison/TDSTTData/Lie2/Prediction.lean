import GNC.Applications.OrbitalComparison.TDSTTData.Lie2.Envelope
import GNC.Applications.OrbitalComparison.TDSTTData.Lie2.Kinematics
import GNC.Dynamics.CandidateOrbitExistence
import GNC.Dynamics.CartesianRadiusDefect

/-! Full inverse-square, all-time certificate for the delivered Lie-coordinate
...
theorem acceleration_continuous (x : Vec3) : Continuous (candidateAcceleration x) := by
  ...
  simp_rw [hn]
  exact ((hq.const_smul _).add exactForce_continuous).add
    ((LieRadiusFrame.left x).toContinuousLinearMap.continuous.comp hp)
...
theorem candidate_radius {x : Vec3} {t : ℝ}
  ...
  have hd := (leftAt_nonexpansive x (LieSTTOutput.value3 physicalInput.rho x t)
    (attitude_bound hx).2).trans (displacement_uniform hx ht)
  have h := CartesianRadiusDefect.candidate_radius (JointErrorData.exactReference t)
    (Jacobian.leftAt x (LieSTTOutput.value3 physicalInput.rho x t)) hd
  rw [JointErrorData.exactReference_norm] at h
  simpa only [candidatePosition,LieRadiusFrame.left,polynomial_value] using h.1
