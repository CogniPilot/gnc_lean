import GNC.Applications.OrbitalComparison.TDSTTData.FullCartesian3.Model
import GNC.Applications.OrbitalComparison.TDSTTData.FullCartesian3.Ranges

/-! The delivered full cubic position and velocity queries, including their
...
def candidatePosition (x : Vec3) (t : ℝ) : E3 :=
  WithLp.toLp 2 (JointErrorData.exactReference t)+PointingCapPolynomial.vectorValue position x t
def candidateVelocity (x : Vec3) (t : ℝ) : E3 :=
  JointErrorData.referenceVelocity t+
    PointingCapPolynomial.vectorValue (PointingCapPolynomial.derivative position) x t
def deliveredVelocity (x : Vec3) (t : ℝ) : E3 :=
  JointErrorData.referenceVelocity t+PointingCapPolynomial.vectorValue velocity x t
...
theorem position_derivative (x : Vec3) (t : ℝ) :
  ...
  (JointErrorData.reference_position_derivative t).add (PointingCapPolynomial.vector_derivative _ _ _)
theorem velocity_derivative (x : Vec3) (t : ℝ) :
    HasDerivAt (candidateVelocity x) (candidateAcceleration x t) t :=
  (JointErrorData.reference_velocity_derivative t).add (PointingCapPolynomial.vector_derivative _ _ _)
...
theorem position_initial (x : Vec3) :
  ...
  simp only [candidatePosition,hi,add_zero]
...
theorem velocity_initial (x : Vec3) : candidateVelocity x 0=JointErrorData.referenceVelocity 0 := by
  have hi : PointingCapPolynomial.vectorValue (PointingCapPolynomial.derivative position) x 0=0 := by
    ...
    simpa only [value_atTime,Rat.cast_zero] using h
  simp only [candidateVelocity,hi,add_zero]
...
theorem delivered_velocity_discrepancy {x : Vec3} {t : ℝ}
  ...
  rw [kinematic_value,polynomial_value]
...
theorem physical_defect_value (x : Vec3) (t : ℝ) :
  ...
  simp only [candidateAcceleration,candidatePosition,thrust,polynomial_value]
  rfl
