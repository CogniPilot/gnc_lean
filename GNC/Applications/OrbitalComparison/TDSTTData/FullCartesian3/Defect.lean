...
import GNC.Applications.OrbitalComparison.TDSTTData.FullCartesian3.Constraint
import GNC.Applications.OrbitalComparison.TDSTTData.FullCartesian3.Residual
import GNC.Applications.OrbitalComparison.TDSTTReference
import GNC.Analysis.QuadraticTimeProfile

/-! The full-field Cartesian acceleration defect of the actual full cubic
...
open ParameterPolynomial LieSTTOutput Planning.PolynomialKernel Set Matrix
...
theorem retained_residual_bound {x : Vec3} {t : ℝ}
    (hx : x 0^2+x 1^2+x 2^2≤(sigma:ℝ)^2) (ht : 0≤t) :
    enorm (value3 (CartesianCubicPolynomial.residual physicalInput) x t)≤
...
theorem residual_bound {x : Vec3} {t : ℝ}
    ...
    enorm (SmallOffsetDefect.full
      ...
      (LieRadiusQuadratic.apply x (x ⨯₃ value3 physicalInput.force x t))
      (physicalInput.K:ℝ) (value physicalInput.h x t))≤
  ...
  rw [CartesianCubicPolynomial.residual_value] at hc
  have h := SmallOffsetDefect.residual_growth
    (value3 (PointingCapPolynomial.derivative (PointingCapPolynomial.derivative physicalInput.rho)) x t)
    (value3 physicalInput.rho x t) (value3 physicalInput.reference x t)
    (LieRadiusQuadratic.apply x (crossProduct x (value3 physicalInput.force x t))) t
...
theorem rawForcing_value (t : ℝ) : PolynomialOrder.value rawForcing t=
    CartesianRadiusDefect.budget (physicalInput.K:ℝ) 1
      ...
      ((sigma:ℝ)*(TDSTTReference.forceBound:ℝ)*(TDSTTReference.phaseBound:ℝ)+
        LieRadiusQuadratic.tail (sigma:ℝ)*(sigma:ℝ)*
  ...
  simp only [CartesianRadiusDefect.budget,constantTail,forceError,gravityFactor,radiusUpper,
    jacobianTail,LieRadiusQuadratic.tail,Gravity.inverseRadiusFactor,
...
theorem physical_defect_profile {x : Vec3} {t : ℝ}
  ...
  have h := CartesianCubicPolynomial.physical_defect physicalInput x t
    (JointErrorData.exactReference t) (JointErrorData.exactForce t)
